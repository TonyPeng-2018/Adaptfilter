...
class resnet_client(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        # See note [TorchScript super()]
        # here we only need the first 4 layers
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
...
class resnet_layers(nn.Module):
    def __init__(
        ...
        self.sections = []
        self.sections.append(self.conv1)
        self.sections.append(self.bn1)
        self.sections.append(self.relu)
        self.sections.append(self.maxpool)
        # combine the layers
        self.sections = self.sections + self.layer1
        self.sections = self.sections + self.layer2
        self.sections = self.sections + self.layer3
        self.sections = self.sections + self.layer4
        self.sections.append(self.avgpool)
        self.sections.append(self.flatten)
