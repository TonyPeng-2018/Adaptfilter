...
def mobilenetV3(num_classes: int = 1000, model_size = 'large') -> MobileNetV3:
    # This function is taken from the original tf repo
    if model_size == 'large':
...
class MobileNetV3_client(nn.Module):
    def __init__(
        self,
        inverted_residual_setting: List[InvertedResidualConfig],
        last_channel: int,
...
class MobileNetV3_server(nn.Module):
    def __init__(
        self,
        inverted_residual_setting: List[InvertedResidualConfig],
        last_channel: int,
