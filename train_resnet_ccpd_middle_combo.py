...
client, server = resnet.resnet_splitter(
    num_classes=34, weight_root="./Weights/ccpd-small", device="cuda:0", layers=50
)

from Dataloaders import dataloader_ccpd

train, _, val = dataloader_ccpd.Dataloader_ccpd_integrated(
    train_batch=128, test_batch=100, size=data_size, device=data_device
)
...
for epoch in tqdm(range(epochs)):
    ...
    for j in range(len(middle_size)):
        ...
        if val_loss[j] < min_val_loss[j]:
            ...
            torch.save(
                middle.state_dict(),
                "resnet_ccpd_" + data_size + "_middle_" + str(middle_size[j]) + ".pth",
            )
