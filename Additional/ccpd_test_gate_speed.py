...
#     device='cpu')
# client = resnet.resnet_splitter_client(num_classes=34, weight_root='./Weights/'+'ccpd-small'+'/', device='cpu', layers=50)
client, server  =resnet.resnet_splitter(num_classes=34, weight_root='./Weights/'+'ccpd-small'+'/',layers=50)
middle_models = []
...
for i in range(len(middle_size)):
    # middle_models.append(mobilenetv2.MobileNetV2_middle(middle=middle_size[i]))
    middle2_models.append(resnet.resnet_middle(middle=middle_size[i]))
gate_models = []
for i in range(len(middle_size)):
    gate_models.append(gatedmodel.ExitGate(in_planes=middle_size[i],
                                           height = height, width=width))
    # load gate
    gate_models[i].load_state_dict(torch.load('./Weights/ccpd-small/gate/resnet_ccpd_gate_'+str(middle_size[i])+'.pth'))
...
images_list = [str(x)+'.bmp' for x in range(100)]
write_file = [data_root+str(x) for x in range(100)]
client_time = 0
