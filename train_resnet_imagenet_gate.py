...
train, _, val, _ = dataloader_image_20.Dataloader_imagenet_20_integrated(train_batch=64, test_batch=50, device='tintin')
import torch
...
middle.load_state_dict(torch.load('resnet_imagenet_middle_'+str(middle_size)+'.pth'))
...
for epoch in tqdm(range(epochs)):
    ...
    if val_acc > max_val_acc:
        max_val_acc = val_acc
        torch.save(gate.state_dict(), 'resnet_imagenet_gate_'+str(middle_size)+'.pth')
        print('model saved')