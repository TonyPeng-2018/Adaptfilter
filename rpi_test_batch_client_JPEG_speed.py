...
for i, i_path in tqdm(enumerate(images_list)):
    ...
    f.write(i_path + ' ' + str(JPEG_time_25[i]) + ' ' + str(JPEG_time_75[i]) + ' ' + str(CJPEG_time[i]) + '\n')
avg_jpeg25 = [0] * i_stop
avg_jpeg75 = [0] * i_stop
avg_cjpeg = [0] * i_stop

for i in range(i_stop):
    avg_jpeg25[i] = sum(JPEG_time_25[i*batch_size:(i+1)*batch_size])/batch_size*1000
    avg_jpeg75[i] = sum(JPEG_time_75[i*batch_size:(i+1)*batch_size])/batch_size*1000
